...
import { writeFileSync, mkdtempSync, writeFileSync as fsWriteFileSync, rmSync, realpathSync } from "fs"
...
async function main() {
  console.log("═══════════════════════════════════════════════════════════")
  console.log("  MCP Developer Tools Test Suite — Trustworthy Edition")
  console.log("═══════════════════════════════════════════════════════════")
  console.log("")

  // Create a fresh temp directory per run to avoid state pollution
  // Use realpathSync to canonicalize /tmp → /private/tmp on macOS
  const tmpDir = realpathSync(mkdtempSync(resolve(tmpdir(), "mcp-dev-test-")))
  fsWriteFileSync(resolve(tmpDir, "config.json"), JSON.stringify({ test: true, benchmark: "mcp" }))

  const allResults = []

  for (const serverDef of SERVERS) {
    // Patch filesystem server to use the fresh temp directory
    const patchedDef = { ...serverDef }
    if (patchedDef.name === "Filesystem MCP") {
      patchedDef.args = [...patchedDef.args]
      patchedDef.args[patchedDef.args.length - 1] = tmpDir
    }

    const adapter = await loadAdapter(serverDef.adapter)
    const harness = new McpTestHarness(patchedDef)
    let result = null
    ...
    try {
      // Pass temp directory as the context path for the adapter
      result = await harness.run(TASK_DEFINITIONS, adapter, tmpDir)
  ...
  const outputPath = resolve(ROOT, "public/data/developer-tools-results.json")
  const report = {
